...
mod_carga_ui <- function(id){
  ...
  shiny::fileInput(ns("carga_final"), "Indique o Diretório do Dataset a Ser Incluído",
                       ...
                       accept = c("text/csv",
                                  ...
                                  ".csv")),
 shiny::actionButton(ns("carga1"), "Executar Carregamento no SQLite"), width = 10))
...
mod_carga_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$carga1, {
      ...
      dm_agente_empresa <- data() %>% dplyr::select(nome_agente_executor,
    ...
    observeEvent(input$carga1, {
      ...
      #Carga dm_projeto
      dm_projeto <- data() %>%
        dplyr::select(id, data_assinatura,data_limite,titulo_projeto,status_projeto)
    ...
    observeEvent(input$carga1,{
      ...
      vlr_res <- data() %>% dplyr::select(id, valor_executado_2013:valor_executado_2020) %>%
      ...
      bs_res <- data() %>% dplyr::select(id, natureza_agente_financiador,
